...
TextStyle mono(double size,
        {FontWeight weight = FontWeight.w500, Color color = ink}) =>
...
class HomeScreen extends StatefulWidget {
  ...
  const HomeScreen(
      {super.key, this.profile, this.onSignOut, this.onAccountDeleted});
  @override
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Column(
                          ...
                          children: [
                        Text(topic['title']!,
                            style: en(13.5, weight: FontWeight.w600)),
                        Text('المستوى ${topic['level']}',
      ...
      HomeContent(
        ...
        onVoice: () async {
          ...
          await Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => VoiceScreen(api: _api, topic: topic)));
        },
        onLevelCheck: () => Navigator.of(context).push(MaterialPageRoute(
            builder: (_) => LevelCheckScreen(
                api: _api,
                userId: _profile?.userId,
                focusSkills: _profile?.focusSkills,
                onComplete: _completeLevelCheck))),
      ),
  ...
  Widget build(BuildContext context) => InkWell(
      ...
      child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(icon, size: 22, color: active ? primary : inkFaint),
            const SizedBox(height: 3),
            Text(label,
                style: ar(11,
                    weight: FontWeight.w600,
                    color: active ? primary : inkFaint))
          ])));
}
...
class _SectionTitle extends StatelessWidget {
  final String text;
  final String? tag;
  const _SectionTitle(this.text, {this.tag});
  @override
  Widget build(BuildContext context) => Padding(
      padding: const EdgeInsets.only(top: 22, bottom: 10),
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(text, style: ar(13, weight: FontWeight.w600, color: inkSoft)),
        if (tag != null)
          Container(
              padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 3),
              decoration: BoxDecoration(
                  color: primaryTint, borderRadius: BorderRadius.circular(20)),
              child: Text(tag!,
                  style: ar(11, weight: FontWeight.w600, color: primary)))
      ]));
}
...
class _Ring extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final progress =
        numeric == null ? 0.0 : (numeric.clamp(0, 100) / 100).toDouble();
    return SizedBox(
      width: 68,
      height: 68,
      child: Stack(
        ...
        children: [
          ...
          Container(
            ...
            height: 50,
            decoration:
                const BoxDecoration(color: paper, shape: BoxShape.circle),
            child: Column(
...
class _HomeContentState extends State<HomeContent> {
  ...
  Widget build(BuildContext context) {
    ...
    final hasLevelResult =
        score > 0 && rawLevel.isNotEmpty && rawLevel != 'pending';
    final hasManualLevel = rawLevel.isNotEmpty &&
        rawLevel != 'pending' &&
        rawLevel != 'intermediate' &&
        !hasLevelResult;
    final hasKnownLevel = hasLevelResult || hasManualLevel;
    final needsLevelCheck =
        !hasKnownLevel && (profile?.focusSkills ?? '').contains('ما أعرف');
    final displayLevel = hasKnownLevel ? profile!.level : 'لم يحدد بعد';
    final name =
        profile?.name.trim().isNotEmpty == true ? profile!.name : 'في حوار';
    final activityTitle =
        errors.isNotEmpty ? 'راجع ملاحظاتك الأخيرة' : 'ابدأ محادثة جديدة';
    final activitySubtitle = errors.isNotEmpty
        ? 'تدريب مبني على كلامك الحقيقي'
        : 'سجّل أول جلسة ليظهر تقدمك هنا';
        ...
        children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Container(
                ...
                decoration: BoxDecoration(
                    color: coralTint, borderRadius: BorderRadius.circular(20)),
                child: Row(children: [
                  const Icon(Icons.calendar_today_outlined,
                      color: Color(0xFFB5451A), size: 14),
                  const SizedBox(width: 5),
                  Text(
                      profile == null || profile.daysSinceJoined == 0
                          ? 'عضو جديد'
                          : 'منذ ${profile.daysSinceJoined} يوم',
                      style: ar(12.5,
                          weight: FontWeight.w600,
                          color: const Color(0xFFB5451A))),
                ])),
          ...
          _Card(
            ...
            Expanded(
                child: Column(
                  ...
                  Text(
                      hasLevelResult
                          ? 'نتيجتك من اختبار تحديد المستوى'
                          : hasManualLevel
                              ? 'المستوى الذي اخترته'
                              : 'لم نحدد مستواك بعد',
                      style: ar(12, color: inkFaint)),
                  const SizedBox(height: 5),
                  Text(
                      hasLevelResult
                          ? '$score نقطة مستوى'
                          : hasManualLevel
                              ? 'يمكنك تغييره من اختبار المستوى'
                              : 'نتيجتك تظهر بعد الاختبار',
                      style:
                          mono(11.5, weight: FontWeight.w600, color: primary)),
                ])),
          ])),
          if (needsLevelCheck) ...[
            ...
            _Card(
                ...
                child: Row(children: [
                  Container(
                      ...
                      child:
                          const Icon(Icons.insights_outlined, color: primary)),
                  const SizedBox(width: 14),
                  Expanded(
                      child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                        Text('خلّنا نحدد مستواك',
                            style: ar(14, weight: FontWeight.w700)),
                        Text('اختبار قصير يختار لك مسارًا مناسبًا',
          ...
          Column(children: [
            ...
            Text(hasTraining ? 'كمل محادثتك الصوتية' : 'ابدأ أول محادثة صوتية',
                style: ar(15.5, weight: FontWeight.w700)),
            const SizedBox(height: 3),
            Text('تحدّث بحرية، والمدرب يستمع ويرد عليك',
                style: ar(12, color: inkFaint)),
          ]),
          if (hasTraining) ...[
            _SectionTitle('تدريبك التالي',
                tag: errors.isNotEmpty ? 'من سجلّك' : 'بعد أول جلسة'),
            _Card(
                onTap: widget.onVoice,
                child: Row(children: [
                  Container(
                  ...
                  Expanded(
                      child: Column(
                        ...
                        Text(activityTitle,
                            style: ar(14, weight: FontWeight.w700)),
                        Text(activitySubtitle, style: ar(12.5, color: inkFaint))
                      ])),
                  const Icon(Icons.chevron_left_rounded, color: inkFaint),
                ])),
              ...
              _Card(
                  onTap: () => Navigator.of(context).push(MaterialPageRoute(
                      builder: (_) => ReviewScreen(
                          api: widget.api, userId: profile?.userId ?? ''))),
                  child: Row(children: [
                    Container(
                        width: 46,
                        height: 46,
                        decoration: BoxDecoration(
                            color: rustTint,
                            borderRadius: BorderRadius.circular(14)),
                        child: const Icon(Icons.refresh_rounded, color: rust)),
                    const SizedBox(width: 14),
                    Expanded(
                        child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                          Text('راجع ملاحظاتك القديمة',
                              style: ar(14, weight: FontWeight.w700)),
                          Text('تدريب متقطع: نرجّع لك الخطأ قبل ما تنساه',
                              style: ar(12.5, color: inkFaint)),
                        ])),
                    const Icon(Icons.chevron_left_rounded, color: inkFaint),
                  ])),
                ...
                child: Row(children: [
                  Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(14)),
                      child: const Icon(Icons.forum_outlined, color: primary)),
                  const SizedBox(width: 14),
                  Expanded(
                      child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                        ...
                        Text('محادثاتك السابقة',
                            style: ar(14, weight: FontWeight.w700)),
                        Text('ارجع لجلستك الأخيرة واقرأ كلامك وتصحيحاته',
                            style: ar(12.5, color: inkFaint)),
                      ])),
                  const Icon(Icons.chevron_left_rounded, color: inkFaint),
                ])),
          ...
          if (errors.isNotEmpty) ...[
            ...
            _Card(
              ...
              Text('ستظهر ملاحظاتك هنا بعد أول تدريب',
                  textAlign: TextAlign.center,
                  style: ar(13, weight: FontWeight.w700)),
              const SizedBox(height: 4),
              Text('لا توجد أخطاء مسجلة بعد، وهذا طبيعي في البداية.',
                  textAlign: TextAlign.center,
                  style: ar(11.5, color: inkFaint)),
            ])),
          ],
...
class _MistakeChip extends StatelessWidget {
  final String wrong, correct, category;
  const _MistakeChip(
      {required this.wrong, required this.correct, required this.category});
  @override
  Widget build(BuildContext context) => Container(
      width: 158,
      ...
      decoration: BoxDecoration(
          color: rustTint,
          border: Border.all(color: const Color(0xFFEAD3CC)),
          borderRadius: BorderRadius.circular(12)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(wrong,
            style: en(12.5, weight: FontWeight.w600, color: rust)
                .copyWith(decoration: TextDecoration.lineThrough)),
        Text(correct,
            style: en(12.5, weight: FontWeight.w600, color: primaryDark)),
        const Spacer(),
        Text(category, style: ar(10.5, color: inkFaint))
      ]));
}
...
class VoiceScreen extends StatefulWidget {
  ...
  const VoiceScreen(
      {super.key, required this.api, this.initialPrompt, this.topic});
...
class _VoiceScreenState extends State<VoiceScreen> {
  ...
  late final Map<String, String> sessionTopic = widget.topic ??
      _conversationTopics[Random().nextInt(_conversationTopics.length)];
  ...
  void initState() {
    ...
    tts.setErrorHandler((message) {
      if (!mounted) return;
      setState(() => status =
          'تعذر تشغيل صوت المدرب من المتصفح. اضغط أيقونة الصوت أو تحقق من مستوى الصوت.');
    });
    _speakOpening();
  }
  ...
  Future<void> _speakOpening() async {
    ...
    final opening = widget.initialPrompt?.trim().isNotEmpty == true
        ? widget.initialPrompt!.trim()
        : 'Hi! Today let\'s talk about: $topicTitle. Tell me your thoughts — take your time and speak freely.';
    ...
    try {
      ...
      if (mounted && !listening && !sending)
        setState(() => status = 'اضغط للبدء بالحديث');
    } catch (_) {
      openingPlayed = false;
      if (mounted)
        setState(
            () => status = 'اضغط للبدء بالحديث؛ سيظهر رد المدرب بعد كلامك.');
    }
  }
  ...
  Future<void> toggleListening() async {
    ...
    if (!available) {
      if (mounted)
        setState(() => status =
            'اسمح للمتصفح باستخدام الميكروفون من رمز القفل بجانب localhost ثم حاول مرة أخرى.');
      return;
    }
    setState(() {
      active = true;
    ...
    try {
      await speech.listen(
        ...
        partialResults: true,
        cancelOnError: true,
        listenFor: const Duration(seconds: 150),
      ...
      if (mounted) {
        setState(() {
          ...
          submittedCurrent = false;
          status =
              'تعذر بدء الاستماع. اسمح بالميكروفون من إعدادات Chrome ثم أعد المحاولة.';
        });
      }
    }
  }
  ...
  Future<void> _setTutorHelp(String? instruction) async {
    setState(() {
      ...
      speechRate = instruction == 'Speak more slowly and use shorter sentences.'
          ? 0.65
          : 0.92;
      status = instruction == null
          ? 'وضع المحادثة الطبيعي'
          : instruction.startsWith('Speak')
              ? 'سأتحدث ببطء أكثر'
              : 'سأبسط شرحي لك';
    });
    try {
  ...
  Future<void> sendTranscript() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        final isMissingAuth =
            error is StateError && error.message == 'AUTH_MISSING';
        ...
        final message = isUnauthorized || isMissingAuth
            ...
            : isTimeout
                ? 'تأخر رد المدرب. تأكد أن Gemini مفعّل ثم حاول مرة أخرى.'
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('${sessionTopic['level']} · ${sessionTopic['title']}',
            style: ar(12, color: inkFaint)),
      ),
      body: LayoutBuilder(
                  ...
                  if (transcript.isNotEmpty) ...[
                    TextButton.icon(
                        ...
                        icon: Icon(
                            ...
                            size: 17),
                        label: Text(
                            showTranscript ? 'إخفاء كلامك' : 'إظهار كلامك',
                            style: ar(11.5, color: primary))),
                    if (showTranscript)
                      Padding(
                    ...
                    child: Wrap(
                        ...
                        children: [
                          ChoiceChip(
                              label: Text('بطّئ لي',
                                  style: ar(11,
                                      color: tutorInstruction != null &&
                                              speechRate < 0.7
                                          ? primary
                                          : inkSoft)),
                              selected:
                                  tutorInstruction != null && speechRate < 0.7,
                              onSelected: sending
                                  ? null
                                  : (_) => _setTutorHelp(tutorInstruction !=
                                              null &&
                                          speechRate < 0.7
                                      ? null
                                      : 'Speak more slowly and use shorter sentences.'),
                              avatar: const Icon(
                                  Icons.slow_motion_video_outlined,
                                  size: 15)),
                          ChoiceChip(
                              label: Text('بسّطها لي',
                                  style: ar(11,
                                      color: tutorInstruction != null &&
                                              speechRate >= 0.7
                                          ? primary
                              ...
                              onSelected: sending
                                  ? null
                                  : (_) => _setTutorHelp(tutorInstruction ==
                                          'Use simpler English and explain your last reply in one short sentence.'
                                      ? null
                                      : 'Use simpler English and explain your last reply in one short sentence.'),
                          ...
                          IconButton(
                              ...
                              icon: Icon(
                                  listening
                                      ? Icons.stop_rounded
                                      : Icons.mic_none,
                                  color: listening ? rust : ink),
                              iconSize: 26,
                              style: IconButton.styleFrom(
                                  backgroundColor: paper,
                                  side: const BorderSide(color: line),
                                  fixedSize: const Size(64, 64))),
                          Text(listening ? 'إيقاف' : 'اضغط وتحدث',
                              style: ar(11.5, color: inkFaint))
                        ]),
                        const SizedBox(width: 34),
                        Column(children: [
                          IconButton(
                              onPressed: finishConversation,
...
class _FeedbackScreenState extends State<FeedbackScreen> {
  ...
  Future<void> _loadHistory() async {
    try {
      ...
      if (userId != null && userId.isNotEmpty)
        historical = await widget.api.getErrors(userId);
      if (mounted) setState(() => loading = false);
    } catch (_) {
      if (mounted)
        setState(() {
          loading = false;
          error = 'تعذر تحميل سجل الأخطاء السابق.';
        });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final current = widget.corrections
        .map((item) => HiwarError(
            wrong: item['wrong'] ?? '',
    ...
    final quizErrors = [...all]
      ..removeWhere((item) {
        ...
        return item.wrong.trim().isEmpty ||
            item.correct.trim().isEmpty ||
            outsideMonth;
      })
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          ...
          elevation: 0,
          title:
              Text('مراجعة المحادثة', style: ar(16, weight: FontWeight.w700))),
      body: ListView(
          ...
          children: [
            ...
            Center(
                child: Container(
                    ...
                    decoration: BoxDecoration(
                        color: primaryTint,
                        borderRadius: BorderRadius.circular(20)),
                    child: Text('✓ انتهت المحادثة',
                        style: ar(12,
                            weight: FontWeight.w700, color: primaryDark)))),
            Center(
                child: Padding(
                    padding: const EdgeInsets.only(top: 12),
                    child: Text(
                        !widget.analysisCompleted
                            ? 'لم يكتمل تحليل الجلسة'
                            : all.isEmpty
                                ? 'لم تظهر أخطاء واضحة'
                                : 'أداء جيد اليوم',
            ...
            Center(
                child: Text(
                    '${minutes}د ${seconds}ث · ${widget.exchanges} تبادلات · ${widget.topic}',
                    style: ar(12.5, color: inkFaint))),
            if (widget.reply.isNotEmpty) ...[
              const _SectionTitle('رد المساعد'),
              _Card(
                  child: Text(widget.reply,
                      style: ar(13, color: inkSoft).copyWith(height: 1.7)))
            ],
            if (widget.tips.isNotEmpty) ...[
              const _SectionTitle('نصيحة لك'),
              _Card(
                  child: Text(widget.tips.join('\n'),
                      style: ar(13, color: inkSoft).copyWith(height: 1.7)))
            ],
            if (loading)
              const Padding(
            ...
            if (error != null)
              Padding(
                  padding: const EdgeInsets.only(top: 12),
                  child: Text(error!,
                      textAlign: TextAlign.center, style: ar(12, color: rust))),
            if (!widget.analysisCompleted)
              _Card(
                  child: Text(
                      widget.analysisMessage ??
                          'لم يصل تحليل حقيقي لهذه الجلسة. فعّل مزود الذكاء الاصطناعي ثم أعد المحاولة.',
                      textAlign: TextAlign.center,
                      style: ar(13, color: inkSoft).copyWith(height: 1.7))),
            if (!loading && widget.analysisCompleted && all.isNotEmpty) ...[
              _SectionTitle('الأخطاء التي تحتاج مراجعة (${historical.length})'),
              ...all.asMap().entries.map((entry) => _ReviewCard(
                  number: '${entry.key + 1}',
                  wrong: entry.value.wrong,
                  correct: entry.value.correct,
                  explain:
                      '${entry.value.explanation}${entry.value.count > 1 ? ' · تكررت ${entry.value.count} مرات' : ''}')),
            ],
            if (!loading && widget.analysisCompleted && all.isEmpty)
              _Card(
                  child: Text(
                      'ممتاز. لم تظهر أخطاء واضحة في هذه الجلسة، وسيظهر أي خطأ حقيقي هنا بعد تحليله.',
                      textAlign: TextAlign.center,
                      style: ar(13, color: inkSoft).copyWith(height: 1.7))),
            if (!loading && quizItems.isNotEmpty) ...[
              ...
              _PersonalizedPracticeCard(
                ...
                onStart: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (_) =>
                            PersonalizedErrorQuizScreen(errors: quizItems))),
              ),
            ],
            const SizedBox(height: 22),
            FilledButton.icon(
          ...
          children: [
            Row(
              children: [
                CircleAvatar(
                    radius: 12,
                    backgroundColor: rustTint,
                    child: Text(number, style: mono(11, color: rust))),
                const SizedBox(width: 10),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('"$wrong"',
                          style: en(13.5, color: rust).copyWith(
                              decoration: TextDecoration.lineThrough)),
                      Text('✓ $correct',
                          style: en(13.5,
                              weight: FontWeight.w600, color: primaryDark)),
                    ],
                  ),
                ),
              ],
            ...
            Text(explain,
                style: ar(12.5, color: inkSoft).copyWith(height: 1.7)),
          ],
        ),
      ),
    );
...
class _ExploreContentState extends State<ExploreContent> {
  ...
  int _pct(int value, int max) =>
      value <= 0 ? 0 : (value * 100 ~/ max).clamp(0, 100);
    ...
    return ListView(
        ...
        children: [
          ...
          Container(
              ...
              decoration: BoxDecoration(
                  gradient:
                      const LinearGradient(colors: [primaryDark, primary]),
                  borderRadius: BorderRadius.circular(18)),
              child: Row(children: [
                Expanded(
                    child: Column(
                      ...
                      Text(
                          suggestion ??
                              'أكمل أول محادثة وسيظهر اقتراحك هنا مبني على كلامك الحقيقي.',
                          style:
                              ar(13, color: Colors.white).copyWith(height: 1.6))
                    ])),
                const SizedBox(width: 8),
                _SparkleArt(),
              ])),
          ...
          _Card(
              onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => JournalScreen(api: widget.api))),
              child: Row(children: [
                Container(
                    width: 46,
                    height: 46,
                    ...
                    child: const Icon(Icons.edit_note_rounded,
                        color: Color(0xFFB5451A))),
                const SizedBox(width: 12),
                Expanded(
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                      Text('يومياتك بالإنجليزي',
                          style: ar(14, weight: FontWeight.w700)),
                      Text('اكتب عن يومك وحوّل كلامك إلى محادثة',
                          style: ar(11.5, color: inkFaint))
                    ])),
                const Icon(Icons.chevron_left_rounded, color: inkFaint)
              ])),
          ...
          _Card(
              onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => GoalPacksScreen(api: widget.api))),
              child: Row(children: [
                Container(
                    width: 46,
                    height: 46,
                    decoration: BoxDecoration(
                        color: primaryTint,
                        borderRadius: BorderRadius.circular(14)),
                    child: const Icon(Icons.flag_outlined, color: primary)),
                const SizedBox(width: 12),
                Expanded(
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                      Text('استعد لهدف قريب',
                          style: ar(14, weight: FontWeight.w700)),
                      Text('خطط قصيرة للسفر ومقابلة العمل وIELTS',
                          style: ar(11.5, color: inkFaint))
                    ])),
                const Icon(Icons.chevron_left_rounded, color: inkFaint)
              ])),
...
class ProgressContent extends StatelessWidget {
  ...
  const ProgressContent(
      {super.key, required this.api, this.profile, this.onLevelCheck});
  @override
  Widget build(BuildContext context) {
    final score = profile?.levelScore ?? 0;
    final level = score > 0 &&
            profile?.level.trim().isNotEmpty == true &&
            profile!.level != 'pending'
        ? profile!.level
        : 'لم يحدد بعد';
    ...
    return ListView(
        ...
        children: [
          ...
          _Card(
              child: Row(children: [
            _Ring(value: score > 0 ? '$score%' : '—', label: 'المستوى'),
            const SizedBox(width: 16),
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(level, style: ar(14.5, weight: FontWeight.w700)),
              Text(days == 0 ? 'ابدأ أول جلسة لك' : 'عضو منذ $days يوم',
                  style: ar(12, color: inkFaint))
            ])
          ])),
          ...
          _Card(
              onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (_) => VoiceTimelineScreen(api: api))),
              child: Row(children: [
                Container(
                    width: 46,
                    height: 46,
                ...
                Expanded(
                    child: Column(
                      ...
                      Text('اسمع تطورك',
                          style: ar(14, weight: FontWeight.w700)),
                      Text('قارن آخر محادثة بأول خطوة لك',
                          style: ar(11.5, color: inkFaint))
                    ])),
                const Icon(Icons.chevron_left_rounded, color: inkFaint)
              ])),
              ...
              child: Row(children: [
                ...
                Expanded(
                    child: Column(
                      ...
                      Text('راجع ملاحظاتك القديمة',
                          style: ar(14, weight: FontWeight.w700)),
                      Text('تدريب متقطع يخليك ما تنسى تصحيحاتك',
                          style: ar(11.5, color: inkFaint))
                    ])),
                const Icon(Icons.chevron_left_rounded, color: inkFaint)
              ])),
          ...
          _Card(
              onTap: onLevelCheck,
              child: Row(children: [
                const _LevelMeterArt(size: 58),
                const SizedBox(width: 12),
                Expanded(
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                      Text('حدد مستواك من جديد',
                          style: ar(14, weight: FontWeight.w700)),
                      Text('اختبار قصير يعطيك مسارًا أدق',
                          style: ar(11.5, color: inkFaint))
                    ])),
          ...
          if (score > 0) ...[
            ...
            _Card(
                onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (_) => AchievementScreen(profile: profile!))),
                child: Row(children: [
                  Container(
                      width: 46,
                      height: 46,
                  ...
                  Expanded(
                      child: Column(
                        ...
                        Text('بطاقة إنجازك',
                            style: ar(14, weight: FontWeight.w700)),
                        Text('احفظها أو شارك تقدمك مع الآخرين',
                            style: ar(11.5, color: inkFaint))
                      ])),
                  const Icon(Icons.chevron_left_rounded, color: inkFaint)
                ])),
          ...
          _Card(
              child: ListTile(
                  ...
                  title: const Text('سجل أخطائك يظهر هنا'),
                  subtitle: Text('بعد أول محادثة ستجد ملاحظاتك الحقيقية هنا',
                      style: ar(11.5, color: inkFaint)))),
        ]);
  }
}
...
class _ProfileContentState extends State<ProfileContent> {
    ...
    try {
      if (value && !ReminderService.supported) {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('التذكيرات مدعومة على تطبيق الهاتف حالياً.')));
        }
        return;
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(value
                ? 'راح يوصلك تذكير يومي 8 مساءً 🎙️'
  ...
  Future<void> _chooseVoice() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
              'تم اختيار ${selected == 'male' ? 'صوت ولد' : 'صوت بنت'} للمدرب.')));
    }
  }
  ...
  TableRow _profileTableRow(String label, String? value) => TableRow(children: [
        Padding(
            ...
            child: Text(label, style: ar(12, color: inkFaint))),
        Padding(
            padding: const EdgeInsets.symmetric(vertical: 9),
  ...
  Future<void> _editProfile() async {
    ...
    final updated = await Navigator.push<HiwarProfile>(
      context,
      MaterialPageRoute(
          builder: (_) => ProfileEditScreen(api: widget.api, profile: current)),
    );
    ...
    try {
        ...
        builder: (dialogContext) => Directionality(
          ...
          child: AlertDialog(
            title: Text('حذف الحساب نهائيًا',
                style: ar(17, weight: FontWeight.w800, color: rust)),
            content: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(
                      'سيتم حذف حسابك وجميع المحادثات والرسائل والأخطاء ونتائج التدريب من قاعدة البيانات. لا يمكن التراجع عن هذا الإجراء.',
                      style: ar(13, color: inkSoft)),
                  const SizedBox(height: 14),
                  Text('اكتب «حذف» للتأكيد',
                      style: ar(12, weight: FontWeight.w700, color: ink)),
                  const SizedBox(height: 7),
                  TextField(
                      ...
                      decoration: const InputDecoration(
                          border: OutlineInputBorder(), hintText: 'حذف')),
                ]),
            actions: [
              TextButton(
                  onPressed: () => Navigator.pop(dialogContext, false),
                  child: Text('إلغاء',
                      style: ar(13, weight: FontWeight.w700, color: inkSoft))),
              ValueListenableBuilder<TextEditingValue>(
                ...
                builder: (_, value, __) => FilledButton(
                  onPressed: value.text.trim() == 'حذف'
                      ...
                      : null,
                  style: FilledButton.styleFrom(
                      backgroundColor: rust, foregroundColor: Colors.white),
                  child: Text('حذف نهائيًا',
                      style:
                          ar(13, weight: FontWeight.w700, color: Colors.white)),
                ),
              ),
            ],
          ),
      ...
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تم حذف الحساب نهائيًا.')));
      } catch (_) {
        if (mounted) {
          setState(() => loading = false);
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text(
                  'تعذر حذف الحساب. لم يتم مسح أي بيانات، حاول مرة أخرى.')));
        }
      }
    } finally {
        ...
        child: AlertDialog(
          ...
          content: SingleChildScrollView(
              child: Column(
                  ...
                  children: [
                Text(
                    'بياناتك ملكك وحدك. في حوار نجمع أقل قدر ممكن من المعلومات:',
                    style: ar(13, color: inkSoft).copyWith(height: 1.7)),
                const SizedBox(height: 10),
                Text('• الاسم والبريد الإلكتروني لإنشاء حسابك فقط.',
                    style: ar(13, color: inkSoft)),
                const SizedBox(height: 5),
                Text(
                    '• محادثاتك وملاحظاتك تستخدم لتخصيص تدريبك ولا تُشارك مع أي طرف ثالث.',
                    style: ar(13, color: inkSoft)),
                const SizedBox(height: 5),
                Text('• لا نبيع بياناتك ولا نستخدمها للإعلانات.',
                    style: ar(13, color: inkSoft)),
                const SizedBox(height: 5),
                Text(
                    '• يمكنك حذف حسابك وكل بياناتك نهائياً في أي وقت من زر «حذف الحساب» بالأسفل.',
                    style: ar(13, color: inkSoft)),
              ])),
          actions: [
            TextButton(
                ...
                child: Text('فهمت',
                    style: ar(13, weight: FontWeight.w700, color: primary)))
          ],
        ),
      ),
    );
      ...
      builder: (dialogContext) => Directionality(
        ...
        child: AlertDialog(
          ...
          content: Column(
              ...
              children: [
                Text(
                    'حوار — مدرب محادثة إنجليزية بالصوت، يتكلم معك ويربط تصحيحاتك بمستواك الحقيقي.',
                    style: ar(13, color: inkSoft).copyWith(height: 1.7)),
                const SizedBox(height: 10),
          ...
          actions: [
            ...
            FilledButton(
              ...
              child: Text('نعم، تسجيل الخروج',
                  style: ar(13, weight: FontWeight.w700, color: Colors.white)),
            ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    final score = profile?.levelScore ?? stats?.levelScore ?? 0;
    final level = score > 0
        ? (profile?.level ?? stats?.level ?? 'لم يحدد بعد')
        : 'لم يحدد بعد';
    ...
    return ListView(
        ...
        children: [
          Align(
              ...
              child: Text('حسابي', style: ar(21, weight: FontWeight.w800))),
          const SizedBox(height: 14),
          _Card(
              child: Column(children: [
            Row(children: [
              ...
              Expanded(
                  child: Column(
                    ...
                    const SizedBox(height: 4),
                    Text(_value(profile?.email),
                        style: en(12, color: inkFaint)),
                    const SizedBox(height: 9),
                    Container(
          ...
          SingleChildScrollView(
              ...
              child: Row(children: [
                _MetricTile(
                    value: profile?.dailyMinutes == null
                        ? '—'
                        : '${profile!.dailyMinutes}',
                    label: 'دقيقة يومية'),
                _MetricTile(
                    value: '${stats?.totalSessions ?? 0}', label: 'محادثة'),
                _MetricTile(
                    value: score > 0 ? '$score' : '—', label: 'نقطة المستوى'),
                _MetricTile(
                    value: '${stats?.streakDays ?? 0}', label: 'يوم متتالٍ'),
              ])),
          ...
          SingleChildScrollView(
              ...
              child: Row(children: [
                _Badge(
                    ...
                    label: 'أيام متتالية',
                    unlocked: (stats?.streakDays ?? 0) > 0),
                const SizedBox(width: 9),
                _Badge(
                    icon: Icons.record_voice_over_outlined,
                    label: 'أول محادثة',
                    unlocked: (stats?.totalSessions ?? 0) > 0),
                const SizedBox(width: 9),
                _Badge(
          ...
          _Card(
            ...
            _TrainingRow(
                ...
                value: profile?.dailyMinutes == null
                    ? 'غير محدد'
                    : '${profile!.dailyMinutes} دقيقة',
                onTap: _editProfile),
            const Divider(height: 1, color: line),
            _TrainingRow(
                icon: Icons.record_voice_over_outlined,
                title: 'صوت المدرب',
                value: voicePreference == 'male' ? 'صوت ولد' : 'صوت بنت',
                onTap: _chooseVoice),
            const Divider(height: 1, color: line),
            _TrainingRow(
                icon: Icons.notifications_none_rounded,
                title: 'تذكير المحادثة اليومية',
                value: reminderEnabled ? 'مفعّل' : 'متوقف',
                trailing:
                    Switch(value: reminderEnabled, onChanged: _toggleReminder),
          ...
          if (!loading && error != null)
            Padding(
                ...
                child: Text('تعذر تحميل الإحصاءات: $error',
                    style: ar(11, color: rust))),
          const SizedBox(height: 26),
          _Card(
            child: Column(children: [
              SizedBox(
              ...
              SizedBox(
                ...
                child: TextButton.icon(
                  ...
                  icon: const Icon(Icons.delete_outline_rounded,
                      color: rust, size: 18),
                  label: Text('حذف الحساب نهائيًا',
                      style: ar(12.5, weight: FontWeight.w700, color: rust)),
                ),
              ),
            ]),
          ),
...
class ProfileEditScreen extends StatefulWidget {
  ...
  const ProfileEditScreen(
      {super.key, required this.api, required this.profile});
...
class _ProfileEditScreenState extends State<ProfileEditScreen> {
  ...
  void initState() {
    ...
    dailyMinutes =
        TextEditingController(text: profile.dailyMinutes?.toString() ?? '');
    education = levels.contains(profile.educationLevel)
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
            ...
            elevation: 0,
            title:
                Text('تعديل معلوماتي', style: ar(17, weight: FontWeight.w800))),
        body: ListView(
            ...
            children: [
              _Card(
                  child: Column(
                    ...
                    const SizedBox(height: 14),
                    TextField(
                        controller: name, decoration: _decoration('الاسم')),
                    const SizedBox(height: 11),
                    TextField(
                        controller: age,
                        keyboardType: TextInputType.number,
                        decoration: _decoration('العمر')),
                    const SizedBox(height: 11),
                    DropdownButtonFormField<String>(
                        value: education,
                        decoration: _decoration('المرحلة الدراسية'),
                        items: levels
                            .map((item) => DropdownMenuItem(
                                value: item, child: Text(item, style: ar(13))))
                            .toList(),
                        onChanged: (value) {
                          if (value != null) setState(() => education = value);
                        }),
                    const SizedBox(height: 11),
                    TextField(
                        controller: dailyMinutes,
                        keyboardType: TextInputType.number,
                        decoration: _decoration('كم دقيقة تتعلم يوميًا؟')),
                    const SizedBox(height: 11),
                    TextField(
                        controller: learningReason,
                        maxLines: 2,
                        decoration: _decoration('لماذا تريد تعلم الإنجليزية؟')),
                    const SizedBox(height: 11),
                    TextField(
                        controller: focusSkills,
                        maxLines: 2,
                        decoration: _decoration('المهارات التي تريد تطويرها')),
                  ])),
              const SizedBox(height: 18),
              FilledButton(
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        SizedBox(
            width: 112, child: Text(label, style: ar(12, color: inkFaint))),
        Expanded(
            child: Text(
                value == null || value!.trim().isEmpty ? 'غير محدد' : value!,
                style: ar(12.5, weight: FontWeight.w600, color: inkSoft)))
...
class _AccountStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _Card(
        ...
        Text(value, style: mono(17, weight: FontWeight.w600, color: primary)),
        Text(label, style: ar(11, color: inkFaint))
      ]));
}
...
class _MetricTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 4),
          Text(label,
              textAlign: TextAlign.center, style: ar(10, color: inkFaint)),
        ]),
      );
}
      ...
      child: SizedBox(
        ...
        child: Row(children: [
          ...
          if (trailing != null)
            ...
            Text(value, style: en(12, color: inkSoft)),
          const SizedBox(width: 8),
  ...
  Widget build(BuildContext context) => SizedBox(
      ...
      child: Stack(alignment: Alignment.center, children: [
        ...
        const Positioned(
            ...
            left: 52,
            child:
                Icon(Icons.star_rounded, color: Color(0xFFD9581F), size: 17)),
        const Positioned(
            top: 46,
            right: 48,
            child:
                Icon(Icons.auto_awesome, color: Color(0xFFB5842B), size: 15)),
        const Positioned(
            bottom: 20,
            right: 68,
            child:
                Icon(Icons.star_rounded, color: Color(0xFFD9581F), size: 12)),
        Positioned(
            bottom: 26,
            left: 62,
            child: Container(
      ...
      child: Stack(
        ...
        children: [
          ...
          Positioned(
              ...
              left: size * .12,
              right: size * .12,
              child: Container(height: 2, color: primaryDark.withOpacity(.35))),
          Positioned(
              top: 2,
              right: size * .18,
              child: Icon(Icons.star_rounded,
                  color: const Color(0xFFD9581F), size: size * .26)),
        ],
      ),
    );
  }
...
class _Badge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
      ...
      decoration: BoxDecoration(
          color: unlocked ? const Color(0xFFF7EEDB) : paper,
          border: Border.all(color: unlocked ? const Color(0xFFEAD9C4) : line),
          borderRadius: BorderRadius.circular(12)),
      child: Column(children: [
        Icon(icon,
            size: 22, color: unlocked ? const Color(0xFFB5842B) : inkFaint),
        const SizedBox(height: 5),
        Text(label,
            textAlign: TextAlign.center,
            style: ar(9.5, weight: FontWeight.w600, color: inkSoft))
      ]));
}
...
class _AnalysisRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(label, style: ar(12, color: inkSoft)),
        Text(value, style: mono(12, weight: FontWeight.w700, color: primary))
      ]));
}
...
class LevelCheckScreen extends StatefulWidget {
  ...
  const LevelCheckScreen(
      {super.key, this.api, this.userId, this.focusSkills, this.onComplete});
  @override
...
class _LevelCheckScreenState extends State<LevelCheckScreen> {
  ...
  final grammar = <Map<String, Object>>[
    ...
    },
    {
      ...
      'a': ['have', 'had', 'will have'],
      'correct': 1
    },
    {
      'q': 'Rarely ___ such dedication in a new learner.',
      'a': ['I have seen', 'have I seen', 'did I saw'],
      'correct': 1
    },
  ];
  final vocabulary = <Map<String, Object>>[
    ...
    {
      'q': '“Delicious” food is…',
      'a': ['very tasty', 'very cold', 'very fast'],
      'correct': 0
    },
    {
      'q': 'A “reliable” person is someone you can…',
      'a': ['trust', 'avoid', 'forget'],
      'correct': 0
    },
    {
      'q': '“Meticulous” means…',
      'a': ['very careful', 'very lazy', 'very loud'],
      'correct': 0
    },
    {
      'q': '“Ubiquitous” describes something…',
      'a': ['found everywhere', 'extremely rare', 'very expensive'],
      'correct': 0
    },
  ];
  ...
  void initState() {
    ...
    listeningOptions = aiChoosesSkill
        ? [
            'They are practicing ordering coffee.',
            'They are discussing a flight cancellation.',
    ...
    videoController =
        VideoPlayerController.asset('assets/hiwar-english-conversation.mp4');
  ...
  List<Map<String, Object>> get currentQuestions =>
      (section == 0 ? grammar : vocabulary);
  ...
  Future<void> startSpeaking() async {
    ...
    final ready = await speech.initialize(
      ...
      onError: (error) {
        ...
        setState(() => listening = false);
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('تعذر تشغيل المايك: ${error.errorMsg}')));
      },
    );
    if (!ready) {
      if (mounted)
  ...
  Future<void> finish() async {
    ...
    try {
      ...
      speakingAnalysis = {
        ...
        'feedback':
            'تعذر الاتصال بتحليل الذكاء الاصطناعي. يمكنك إعادة المحاولة لاحقًا.'
      };
    }
    final remoteLevel =
        '${speakingAnalysis['estimated_level'] ?? ''}'.trim().toUpperCase();
    final rawSpeakingScore = speakingAnalysis['overall_score'];
    final speakingScore = rawSpeakingScore is num
        ? rawSpeakingScore.toInt()
        : int.tryParse('$rawSpeakingScore');
    final knowledgeScore = ((score / 12) * 100).round().clamp(0, 100).toInt();
    final finalScore = speakingScore == null || speakingScore <= 0
        ? knowledgeScore
        : ((knowledgeScore * .5) + (speakingScore * .5))
            .round()
            .clamp(0, 100)
            .toInt();
    ...
    setState(() {
      ...
      showResult = true;
      if (saveError != null)
        speakingAnalysis = {...speakingAnalysis, 'save_error': saveError};
    });
  }
    ...
    return Expanded(
      child: Container(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 4),
          Text(label,
              textAlign: TextAlign.center, style: ar(11, color: inkFaint)),
        ]),
      ),
    );
  }
        ...
        children: [
          Row(children: [
            IconButton(
                ...
                icon: const Icon(Icons.arrow_back_ios_new_rounded,
                    size: 19, color: ink)),
            Expanded(
                child: Text('نتيجة تحديد المستوى',
                    textAlign: TextAlign.center,
                    style: ar(17, weight: FontWeight.w800))),
          ...
          Container(
            ...
            child: Column(children: [
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Center(
                          child: Text('$percent%',
                              style: ar(24,
                                  weight: FontWeight.w800, color: primary))),
                    ),
                  ],
                ),
              ),
              ...
              const SizedBox(height: 4),
              Text('$resultLevel',
                  style: ar(22, weight: FontWeight.w800, color: primary)),
              const SizedBox(height: 10),
              Text(
                  'نتيجتك مبنية على إجاباتك في الاختبار وتحليل Speaking بالذكاء الاصطناعي.',
                  textAlign: TextAlign.center,
                  style: ar(12.5, color: inkSoft).copyWith(height: 1.7)),
            ]),
          ),
          ...
          Row(children: [
            ...
            const SizedBox(width: 10),
            _resultMetric(
                'Speaking', spokenText.trim().isEmpty ? 'غير مكتمل' : 'مكتمل'),
          ]),
          const SizedBox(height: 16),
          _Card(
              child: Column(
                ...
                if (speakingAnalysis['save_error'] != null) ...[
                  const SizedBox(height: 8),
                  Text('${speakingAnalysis['save_error']}',
                      style: ar(11.5, color: rust).copyWith(height: 1.6)),
                ],
                const SizedBox(height: 8),
                Text(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: AppBar(
            ...
            elevation: 0,
            title:
                Text('تحديد المستوى', style: ar(16, weight: FontWeight.w700))),
        body: ListView(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              Text('المرحلة ${section + 1} من $totalSections',
                  textAlign: TextAlign.center,
                  style: ar(11.5, color: inkFaint, weight: FontWeight.w600)),
              const SizedBox(height: 18),
              Center(
                  child: Text(
                      isReading
                          ...
                          : isListening
                              ...
                              : isSpeaking
                                  ...
                                  : section == 0
                                      ? 'Grammar'
                                      : 'Vocabulary',
                      style: en(18, weight: FontWeight.w800, color: primary))),
              const SizedBox(height: 14),
              if (section < 2) ...[
                Text('السؤال ${question + 1} من 5',
                    textAlign: TextAlign.center,
                    style: ar(12, color: inkFaint)),
                const SizedBox(height: 16),
                _Card(
                ...
                Text('اقرأ القطعة ثم أجب عن السؤال.',
                    style: ar(14, weight: FontWeight.w700)),
                const SizedBox(height: 12),
                _Card(
                    child: Text(
                ...
                _Card(
                  ...
                  Text(
                      aiChoosesSkill
                          ? 'اختار لك الـAI سؤالًا مختلفًا يناسب تقييمك. شاهد المقطع ثم اختر المعنى الأقرب.'
                          : 'شاهد المقطع ثم اختر الجملة التي تصف ما سمعته في الحوار.',
                      textAlign: TextAlign.center,
                      style: ar(13, color: inkSoft).copyWith(height: 1.6)),
                  const SizedBox(height: 12),
                  ClipRRect(
                    borderRadius: BorderRadius.circular(16),
                    child: videoReady
                        ...
                        : Container(
                            ...
                            child: Center(
                                child: videoFailed
                                    ? Text(
                                        'تعذر تحميل الفيديو. تحققي من اتصال الإنترنت.',
                                        textAlign: TextAlign.center,
                                        style: ar(12, color: rust))
                  ...
                  const SizedBox(height: 10),
                  FilledButton.icon(
                      onPressed: videoReady ? playListeningClip : null,
                      icon: Icon(playingListening
                          ? Icons.pause_rounded
                          : Icons.play_arrow_rounded),
                      label: Text(
                          playingListening ? 'إيقاف الفيديو' : 'تشغيل الفيديو',
                          style: ar(12, weight: FontWeight.w700)),
                      style: FilledButton.styleFrom(
                          backgroundColor: primaryTint,
                          foregroundColor: primary)),
                  const SizedBox(height: 10),
                  ...listeningOptions.map((item) => ListTile(
                      contentPadding: EdgeInsets.zero,
                      title: Text(item, style: en(13)),
                      onTap: () {
                        answeredQuestions++;
                ...
                _Card(
                  ...
                  const SizedBox(height: 16),
                  IconButton(
                      onPressed: startSpeaking,
                      icon: Icon(
                          listening ? Icons.stop_circle : Icons.mic_rounded,
                          color: listening ? rust : primary,
                          size: 58)),
                  if (spokenText.isNotEmpty) ...[
                    TextButton.icon(
                        onPressed: () =>
                            setState(() => showSpokenText = !showSpokenText),
                        icon: Icon(
                            showSpokenText
                                ? Icons.visibility_off_outlined
                                : Icons.visibility_outlined,
                            size: 17),
                        label: Text(
                            showSpokenText ? 'إخفاء كلامك' : 'إظهار كلامك',
                            style: ar(11.5, color: primary))),
                    if (showSpokenText)
                      Text(spokenText,
                          textAlign: TextAlign.center,
                          style: en(13, color: inkSoft).copyWith(height: 1.6)),
                  ],
                  const SizedBox(height: 8),
                  Text(
                      'Grammar · Vocabulary · Fluency · Pronunciation · Naturalness',
                      textAlign: TextAlign.center,
                      style: ar(11, color: inkFaint))
                ])),
                const SizedBox(height: 18),
                FilledButton(
                    onPressed:
                        analyzing || spokenText.trim().isEmpty ? null : finish,
                    style: FilledButton.styleFrom(
                        backgroundColor: primary,
                        padding: const EdgeInsets.all(16)),
                    child: Text(
...
class _PersonalizedPracticeCard extends StatelessWidget {
  ...
  const _PersonalizedPracticeCard(
      {required this.errors, required this.onStart});
    ...
    return _Card(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(
              '${errors.length} ${errors.length == 1 ? 'سؤال اختاره' : 'أسئلة اختارها'} لك النظام من أكثر أخطائك تكرارًا هذا الشهر.',
              style: ar(12.5, color: inkSoft).copyWith(height: 1.6)),
          const SizedBox(height: 8),
          Text('من: ${_practiceTypeLabel(top.errorType)} ×${top.count}',
              style: ar(11.5, color: primary, weight: FontWeight.w700)),
          const SizedBox(height: 12),
          FilledButton.icon(
              onPressed: onStart,
              icon: const Icon(Icons.play_arrow_rounded),
...
class PersonalizedErrorQuizScreen extends StatefulWidget {
  ...
  @override
  State<PersonalizedErrorQuizScreen> createState() =>
      _PersonalizedErrorQuizScreenState();
}

class _PersonalizedErrorQuizScreenState
    extends State<PersonalizedErrorQuizScreen> {
  int index = 0;
  ...
  List<String> get options {
    ...
    if (values[0].trim() == values[1].trim())
      values[1] = '${current.wrong} (كما قلتها سابقًا)';
    return values;
  }
    ...
    if (index == widget.errors.length - 1) {
      showDialog<void>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text(
              'أجبت صح على $correctAnswers من ${widget.errors.length} أسئلة.',
              style: ar(14, color: inkSoft)),
          actions: [
            TextButton(
                onPressed: () {
                ...
                child: Text('تم',
                    style: ar(13, color: primary, weight: FontWeight.w700)))
          ],
        ),
      );
      return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          backgroundColor: bg,
          elevation: 0,
          title: Text('تمرين مخصص لك', style: ar(16, weight: FontWeight.w800))),
      body: ListView(
          ...
          Row(
              children: List.generate(
                  ...
                  (i) => Expanded(
                      child: Container(
                          margin: EdgeInsets.only(
                              left: i == widget.errors.length - 1 ? 0 : 5),
                          height: 6,
                          decoration: BoxDecoration(
                              color: i <= index ? primary : line,
                              borderRadius: BorderRadius.circular(6)))))),
          const SizedBox(height: 22),
          Text('السؤال ${index + 1} من ${widget.errors.length}',
              style: ar(12, color: inkFaint)),
          const SizedBox(height: 10),
          _Card(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                Text('من: ${_practiceTypeLabel(item.errorType)} ×${item.count}',
                    style: ar(12, color: primary, weight: FontWeight.w700)),
                const SizedBox(height: 12),
                Text('اختر التصحيح الصحيح للجملة أو العبارة:',
                    style: ar(14, weight: FontWeight.w800)),
                const SizedBox(height: 8),
                Text('"${item.wrong}"',
                    style: en(18, color: inkSoft)
          ...
          ...options.map((option) {
            ...
            final color = !answered
                ...
                : isCorrect
                    ...
                    : isSelected
                        ? const Color(0xFFF4C3C0)
                        : line;
            return Padding(
                padding: const EdgeInsets.only(bottom: 10),
                child: OutlinedButton(
                    onPressed: answered ? null : () => answer(option),
                    style: OutlinedButton.styleFrom(
                    ...
                    child: Align(
                        alignment: Alignment.centerRight,
                        child: Text(option, style: en(14, color: ink)))));
          }),
          if (answered) ...[
            const SizedBox(height: 4),
            _Card(
                child: Text(
                    selected == item.correct
                        ? 'صح! ${item.explanation.isEmpty ? 'ممتاز، استمر على هذا الاستخدام.' : item.explanation}'
                        : 'راجعها مرة ثانية: ${item.explanation.isEmpty ? 'حاول استخدام الصياغة الصحيحة في محادثتك القادمة.' : item.explanation}',
                    style: ar(13, color: inkSoft).copyWith(height: 1.7))),
            const SizedBox(height: 14),
            FilledButton(
                onPressed: next,
  ...
  Future<void> _analyze() async {
    ...
    try {
      ...
      if (mounted)
        setState(() {
          busy = false;
          error = HiwarApi.describeError(_);
        });
    }
  }
            ...
            children: [
              _Card(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                    Text('اكتب 2–3 جمل عن يومك',
                        style: ar(16, weight: FontWeight.w800)),
                    const SizedBox(height: 6),
                    Text('سنصحح كلامك ونحوّله إلى سؤال طبيعي للمحادثة.',
                        style: ar(12, color: inkFaint)),
                    const SizedBox(height: 14),
                    TextField(
                        controller: entry,
              ...
              if (result != null) ...[
                ...
                _Card(
                    child: Column(
                      ...
                      Text(result!.originalText,
                          style: en(13, color: rust).copyWith(height: 1.5)),
                      ...
                      if (result!.corrections.isNotEmpty) ...[
                        ...
                        for (final correction in result!.corrections)
                          Text(
                              ...
                              style:
                                  ar(12, color: inkSoft).copyWith(height: 1.6)),
                      ],
                    ])),
                ...
                _Card(
                  ...
                  Text(result!.followUpQuestion,
                      style: en(14, weight: FontWeight.w600, color: inkSoft)
                          .copyWith(height: 1.6)),
                  const SizedBox(height: 12),
                  SizedBox(
                      width: double.infinity,
                      child: FilledButton.icon(
                          ...
                          label: Text('تحدث عن هذا الموضوع',
                              style: ar(13, weight: FontWeight.w700)),
                          style:
...
class GoalPacksScreen extends StatelessWidget {
  ...
  static const packs = <Map<String, String>>[
    {
      'title': 'مقابلة عمل',
      'subtitle': 'خطة 7 أيام · تحدث عن خبرتك بثقة',
      'icon': '💼'
    },
    {
      'title': 'السفر',
      'subtitle': 'خطة 5 أيام · المطار والفندق والمطعم',
      'icon': '✈️'
    },
    {
      'title': 'IELTS Speaking',
      'subtitle': 'خطة 7 أيام · تدرب على إجابات واضحة',
      'icon': '🎯'
    },
  ];
                        ...
                        child: Row(children: [
                          ...
                          Expanded(
                              child: Column(
                                  ...
                                  children: [
                                Text(pack['title']!,
                                    style: ar(14.5, weight: FontWeight.w800)),
                                const SizedBox(height: 4),
                                Text(pack['subtitle']!,
                                    style: ar(11.5, color: inkFaint))
                              ])),
...
class AchievementScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final message =
        'حققت مستوى ${profile.level} في Hiwar بنسبة ${profile.levelScore}%.';
    return Scaffold(
        backgroundColor: bg,
        appBar: AppBar(
            backgroundColor: bg,
            elevation: 0,
            title: Text('بطاقة إنجاز', style: ar(16, weight: FontWeight.w700))),
        body: ListView(
              ...
              _Card(
                  child: Container(
                      ...
                      child: Column(children: [
                        ...
                        const SizedBox(height: 12),
                        Text('إنجاز جديد', style: ar(13, color: Colors.white)),
                        const SizedBox(height: 8),
                        Text(profile.name,
                            style: ar(22,
                                weight: FontWeight.w800, color: Colors.white)),
...
class PersonalInfoScreen extends StatelessWidget {
  ...
  TableRow _row(String label, String? value) => TableRow(children: [
        Padding(
            ...
            child: Text(label, style: ar(12.5, color: inkFaint))),
        Padding(
            padding: const EdgeInsets.symmetric(vertical: 10),
            child: Text(_value(value),
                style: ar(13, weight: FontWeight.w600, color: inkSoft))),
      ]);
        ...
        body: ListView(
            ...
            children: [
                          ...
                          child: Column(
                            ...
                            Text(
                                profile?.name.trim().isNotEmpty == true
                                    ? profile!.name
                                    : 'بدون اسم',
                                style: ar(15.5, weight: FontWeight.w800)),
                            Text(_value(profile?.email),
                                style: en(12, color: inkFaint)),
                          ])),
                    ]),
              ...
              FilledButton.icon(
                ...
                label: Text('رجوع',
                    style:
                        ar(14, weight: FontWeight.w700, color: Colors.white)),
                style: FilledButton.styleFrom(
                    backgroundColor: primary,
                    padding: const EdgeInsets.symmetric(vertical: 14)),
              ),
