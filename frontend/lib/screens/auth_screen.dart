...
class _AuthScreenState extends State<AuthScreen> {
  ...
  TextStyle arabic(double size,
      {FontWeight weight = FontWeight.w400,
      Color color = const Color(0xFF241F38)}) {
  ...
  InputDecoration fieldDecoration(String label,
    ...
    return InputDecoration(
      ...
      prefixIcon: icon == null
          ? null
          : Icon(icon, color: const Color(0xFF948DA6), size: 20),
      filled: true,
      ...
      focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(15),
          borderSide: const BorderSide(color: Color(0xFF4B3F8F), width: 1.4)),
    );
  }
  ...
  Future<void> _finishGoogle() async {
    try {
      ...
      if (idToken == null || idToken.isEmpty) {
        throw Exception(
            'Google ID token is missing. Configure OAuth client IDs first.');
      }
  ...
  Future<void> _manualSignIn() async {
    ...
    try {
      final profile = await widget.api
          .passwordSignIn(email: email.text.trim(), password: password.text);
      await widget.api.saveUserId(profile.userId);
      if (mounted) widget.onSignedIn(profile);
    } on Exception catch (e) {
      if (mounted)
        setState(() {
          busy = false;
          error = 'تعذر الدخول: ${HiwarApi.describeError(e)}';
        });
    }
  }
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          error = sent
              ? null
              : 'تم إنشاء الحساب، لكن تعذر إرسال رمز التحقق. تحقق من إعدادات البريد في الخادم.';
        });
      }
    } on Exception catch (e) {
      if (mounted)
        setState(() {
          busy = false;
          error = 'تعذر إنشاء الحساب: ${HiwarApi.describeError(e)}';
        });
    }
  }
  ...
  Future<void> _verifySignup() async {
    ...
    try {
      final profile = await widget.api
          .verifyEmail(email: email.text.trim(), code: code.text.trim());
      await widget.api.saveUserId(profile.userId);
      if (mounted) widget.onSignedIn(profile);
    } on Exception catch (e) {
      if (mounted)
        setState(() {
          busy = false;
          error = 'الرمز غير صحيح: ${HiwarApi.describeError(e)}';
        });
    }
  }
  ...
  Widget _socialButton() {
    final style = OutlinedButton.styleFrom(
        ...
        side: const BorderSide(color: Color(0xFFE1DBD3)),
    ...
    return Column(children: [
      SizedBox(
          ...
          child: OutlinedButton.icon(
              onPressed: busy ? null : _finishGoogle,
              icon: const Text('G',
                  style: TextStyle(
                      fontSize: 20,
                      fontWeight: FontWeight.w800,
                      color: Color(0xFF4285F4))),
              label: Text('المتابعة باستخدام Google',
                  style: arabic(13.5, weight: FontWeight.w700)),
              style: style)),
      const SizedBox(height: 10),
      SizedBox(
          width: double.infinity,
          child: OutlinedButton.icon(
              onPressed: busy ? null : _finishApple,
              icon: const Text('',
                  style: TextStyle(fontSize: 22, color: Colors.black)),
              label: Text('المتابعة باستخدام Apple',
                  style: arabic(13.5, weight: FontWeight.w700)),
              style: style)),
    ]);
  }
            ...
            child: ListView(
              ...
              Center(
                  child: Text(
                      ...
                      style: arabic(19, weight: FontWeight.w800))),
              const SizedBox(height: 6),
              Center(
                  child: Text(
                      waitingForCode
                          ? 'تحقق من بريدك الإلكتروني لنكمل معًا.'
                          : (signupMode
                              ? 'احفظ تقدمك وخل تجربتك مصمم لك.'
                              : 'سجّل دخولك عشان تكمل رحلتك بالإنجليزي'),
                      textAlign: TextAlign.center,
                      style: arabic(12.5, color: const Color(0xFF948DA6))
                          .copyWith(height: 1.6))),
              ...
              Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                            color: const Color(0xFF32265C).withOpacity(.06),
                            blurRadius: 24,
                  ...
                  child: Column(
                      ...
                      children: [
                        if (!signupMode && !waitingForCode) ...[
                          ...
                          Row(children: [
                            const Expanded(
                                child: Divider(color: Color(0xFFE8E2DB))),
                            Padding(
                                padding:
                                    const EdgeInsets.symmetric(horizontal: 10),
                                child: Text('أو',
                                    style: arabic(11,
                                        color: const Color(0xFF948DA6)))),
                            const Expanded(
                                child: Divider(color: Color(0xFFE8E2DB)))
                          ]),
                          const SizedBox(height: 16),
                          TextField(
                              ...
                              decoration: fieldDecoration('البريد الإلكتروني',
                                  icon: Icons.mail_outline)),
                          const SizedBox(height: 11),
                          TextField(
                              controller: password,
                              obscureText: true,
                              decoration: fieldDecoration('كلمة المرور',
                                  hint: '8 أحرف على الأقل',
                                  icon: Icons.lock_outline)),
                          const SizedBox(height: 14),
                          FilledButton(
                              onPressed: busy ? null : _manualSignIn,
                          ...
                          TextButton(
                              ...
                              child: Text('ما عندك حساب؟ إنشاء حساب بالبريد',
                                  style: arabic(12.5,
                                      weight: FontWeight.w700,
                                      color: primary))),
                        ],
                        if (signupMode || waitingForCode) ...[
                          if (!waitingForCode) ...[
                            ...
                            FilledButton(
                                onPressed: busy ? null : _requestSignupCode,
                                style: FilledButton.styleFrom(
                                    backgroundColor: primary,
                                    padding: const EdgeInsets.all(16),
                                    shape: RoundedRectangleBorder(
                                ...
                                child: Text('إنشاء الحساب وإرسال الرمز',
                                    style: arabic(13.5,
                                        weight: FontWeight.w700,
                                        color: Colors.white))),
                          ] else ...[
                            Container(
                                ...
                                child: Text(
                                    'أرسلنا رمزًا إلى ${email.text.trim()}',
                                    textAlign: TextAlign.center,
                                    style: arabic(12.5, color: primary))),
                            const SizedBox(height: 14),
                            TextField(
                                controller: code,
                                keyboardType: TextInputType.number,
                                textAlign: TextAlign.center,
                                decoration: fieldDecoration('رمز التحقق',
                                    hint: '000000')),
                            const SizedBox(height: 15),
                            FilledButton(
                                onPressed: busy ? null : _verifySignup,
                                style: FilledButton.styleFrom(
                                    backgroundColor: primary,
                                    padding: const EdgeInsets.all(16),
                                    shape: RoundedRectangleBorder(
                                        borderRadius:
                                            BorderRadius.circular(15))),
                                child: Text('تأكيد البريد والدخول',
                                    style: arabic(14,
                                        weight: FontWeight.w700,
                                        color: Colors.white))),
                          ],
              ...
              const SizedBox(height: 16),
              Text('بالاستمرار، أنت توافق على تجربة تعلم آمنة ومحترمة.',
                  textAlign: TextAlign.center,
                  style: arabic(10.5, color: const Color(0xFF948DA6))),
            ])),
      ),
    );
  }
