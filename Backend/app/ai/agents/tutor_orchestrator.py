...
class TutorOrchestrator:
    def __init__(self, db: Session, provider: OpenAITextProvider | None = None):
        self.db = db
        self.provider = provider or OpenAITextProvider()
    ...
    def _record_usage(self, user: User, session: TutorSession, feature: str, usage: ProviderUsage | None) -> None:
        if usage is None:
            return
        self.db.add(
            AIUsageEvent(
    ...
    def _trace(self, session: TutorSession, step: str, status: str, started: float, details: dict) -> None:
        self.db.add(
            AgentTrace(
                session_id=session.id,
                step=step,
